    ...
    return (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6 sm:mb-8">
                <div>
                    <h1 className={pageTitleClass}>🛎️ Уведомления</h1>
                    {user?.shopName && (
                        <p className="text-sm text-gray-500 mt-1">Магазин: {user.shopName}</p>
                    )}
                </div>
                <div className="flex gap-4 text-sm">
                    <Link to="/courier/orders" className="text-pink-600 hover:underline">
                        К доставкам
                    </Link>
                    <button
                        type="button"
                ...
                <div className="space-y-4">
                    {notifications.map(notif => {
                        ...
                        return (
                            <Link
                                key={notif.id}
                                to={notificationTargetLink(notif, { isCourier: true })}
                            ...
                            >
                                <div className="flex items-start gap-4">
                                    <div className="text-2xl sm:text-3xl shrink-0">
                                        {notif.type === 'ORDER' ? '🛒' : notif.type === 'CHAT' ? '💬' : notif.type === 'STOCK' ? '🌸' : '📦'}
                                    </div>
